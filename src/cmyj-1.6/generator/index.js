(() => {
  ...
  const STORAGE_KEY_API = 'canming-gen-api-cfg';
  const STORAGE_KEY_UI = 'canming-gen-ui-cfg';
  const STORAGE_KEY_CUSTOM = 'canming-gen-custom-modules';
  const STORAGE_KEY_ORDER = 'canming-gen-order-list';
...
2-3句在亲密情境中TA会说的话（可以和SFW中的话语方式对照，看同一个人在不同场景下怎么说话）。
  ...
  // 世界书参考
  const STORAGE_KEY_WB = 'canming-gen-wb-sel';
  let curCharName = '';
  let allWbNames = [];
