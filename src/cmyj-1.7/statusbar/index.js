...
function meta(label, value, extraClass = '') {
  const text = value || '未载';
  const expandable = extraClass.includes('cm-meta-long')
    ? ` tabindex="0" aria-label="${html(`${label}：${text}`)}"`
    : '';
  return `<div class="cm-meta ${extraClass}"><span>${html(label)}</span><b title="${html(text)}"${expandable}>${html(text)}</b></div>`;
}
  ...
  return `
    ...
    ${foldGroup(
      ...
      <article class="cm-item cm-comparison-card cm-roster-card">
        ...
        <div class="cm-info-grid">
          ${meta('兵种', camp.兵种 || '未知', 'cm-meta-long')}
          ${meta('将领', camp.将领 || '未定')}
          ${meta('驻地', camp.驻地 || '未载', 'cm-meta-long')}
          ${meta('人数', `${camp.人数 ?? 0} 人`)}
          ${meta('装备', camp.装备 || '未载')}
          ${meta('等级', camp.等级 || '未载')}
        </div>
        <div class="cm-card-metrics">${bar('士气', camp.士气 ?? 0)}${bar('训练', camp.训练 ?? 0)}${bar('后勤', camp.后勤 ?? 0)}</div>
      </article>`,
        '暂无营伍。',
      ),
      '暂无营伍。',
    ...
    ${foldGroup(
      ...
      <article class="cm-item cm-comparison-card cm-metric-card">
        ...
        <div class="cm-mini-bars cm-card-metrics">${['统率', '武力', '智谋', '政治', '威望'].map(key => bar(key, general[key] ?? 0)).join('')}${bar('忠心', findInterpersonalPerson(statData, name)?.person?.忠心 ?? 0)}</div>
      </article>`,
        '暂无将领。',
      ),
      '暂无将领。',
    ...
    ${foldGroup(
      ...
      recordList(
        ...
        (name, battle) => {
          ...
          return `
      <article class="cm-item cm-battle-card">
        ...
        <div class="cm-modal-body">
          ...
          ${
            ...
            <article class="cm-item cm-comparison-card cm-roster-card">
              ...
              <div class="cm-info-grid">
                ${meta('兵种', a.兵种 || '未知', 'cm-meta-long')}
                ${meta('将领', a.将领 || '未定')}
                ${meta('装备', a.装备 || '未载')}
                ${meta('等级', a.等级 || '未载')}
                ${meta('驻地', a.驻地 || '未载', 'cm-meta-long')}
                ${meta('状态', a.状态 || '—')}
              </div>
              <div class="cm-card-metrics">${bar('士气', a.士气 ?? 0)}${bar('训练', a.训练 ?? 0)}${bar('后勤', a.后勤 ?? 0)}</div>
            </article>`,
                    )
                    .join('')}</div>`,
                )
...
function styleText() {
  return `
    *{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;font-family:"Noto Serif SC","Songti SC","SimSun",serif;color:var(--ink);scrollbar-width:thin;scrollbar-color:var(--line) transparent}*::-webkit-scrollbar{width:8px;height:8px}*::-webkit-scrollbar-track{background:transparent}*::-webkit-scrollbar-thumb{background:linear-gradient(var(--line),rgba(164,61,45,.36));border:2px solid transparent;border-radius:999px;background-clip:padding-box}*::-webkit-scrollbar-thumb:hover{background:var(--accent);border:2px solid transparent;background-clip:padding-box}body{background:transparent}.cm-root{width:100%;height:100%;}
    ...
    .cm-meta{display:grid;grid-template-columns:max-content minmax(0,1fr);align-items:start;gap:12px}.cm-meta>b{min-width:0;text-align:right;overflow-wrap:anywhere}.cm-comparison-card{display:grid;grid-template-rows:auto minmax(0,1fr) auto;min-width:0;container-type:inline-size}.cm-comparison-card>.cm-item-title{min-width:0}.cm-comparison-card>.cm-item-title>b{min-width:0;overflow-wrap:anywhere}.cm-comparison-card>.cm-info-grid{align-content:start}.cm-card-metrics{grid-row:3;align-self:end;padding-top:5px}.cm-card-metrics>.cm-bar-row:first-child{margin-top:0}.cm-roster-card .cm-meta-long>b{line-height:1.55}
    .cm-battle-card{display:flex;flex-direction:column;min-width:0;container-type:inline-size}.cm-battle-card>.cm-item-title{align-items:flex-start;min-width:0}.cm-battle-card>.cm-item-title>b{min-width:0;overflow-wrap:anywhere}.cm-battle-card>.cm-item-title .cm-tag{max-width:48%;flex-shrink:0;text-align:center}.cm-battle-card>.cm-info-grid{grid-template-columns:1fr;align-content:start}.cm-battle-card .cm-meta{grid-template-columns:4em minmax(0,1fr)}.cm-battle-card .cm-meta>b{text-align:left;line-height:1.65}.cm-battle-card>.cm-heart{margin:auto 0 0;padding-top:12px}
    @container (min-width:331px) and (max-width:560px){.cm-roster-card .cm-meta-long{min-height:calc(4.65em + 14px)}.cm-roster-card .cm-meta-long>b{display:-webkit-box;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:3}.cm-roster-card .cm-meta-long>b:hover,.cm-roster-card .cm-meta-long>b:focus{-webkit-line-clamp:unset;outline:1px dashed var(--line);outline-offset:3px}}
