...
(() => {
  ...
  const STORAGE_KEY_API = 'canming-1.8:generator:api';
  const STORAGE_KEY_UI = 'canming-1.8:generator:ui';
  const STORAGE_KEY_CUSTOM = 'canming-1.8:generator:custom-modules';
  const STORAGE_KEY_ORDER = 'canming-1.8:generator:order';
...
2-3句在亲密情境中TA会说的话（可以和SFW中的话语方式对照，看同一个人在不同场景下怎么说话）。
  ...
  // 世界书参考
  const STORAGE_KEY_WB = 'canming-1.8:generator:worldbook-selection';
  let curCharName = '';
  let allWbNames = [];
