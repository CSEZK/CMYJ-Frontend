...
(() => {
  ...
  const VERSION = '2.0.0';
  const RUNTIME_KEY = '__CMYJWorldEngineV18';
  const CHAT_STATE_KEY = 'cmyj_world_engine_v18';
  const BACKUP_SCRIPT_ID = 'cmyj-world-engine-backup-v18';
  const BACKUP_STATE_KEY = 'cmyj_world_engine_backups_v18';
  const STORAGE_INTEGRITY_INTERVAL_MS = 2000;
  const INJECTION_ID = 'cmyj-world-engine-context-v18';
  const LAMP_ID = 'canming-world-engine-lamp-v18';
  const FRAME_ID = 'canming-world-engine-frame-v18';
  const BANNER_ID = 'canming-world-engine-banner-v18';
  const STYLE_ID = 'canming-world-engine-lamp-style-v18';
  const STORAGE_PREFIX = 'canming-world-engine-1.8:';
  const STATUSBAR_THEME_KEY = 'canming-afterglow-1.8:statusbar:theme';
