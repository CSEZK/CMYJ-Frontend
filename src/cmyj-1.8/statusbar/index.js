...
const PORTRAIT_LIBRARY_STORAGE_KEY = 'canming-afterglow-1.8:portrait-library-v1';
const CHARACTER_PROFILE_STORAGE_KEY = 'canming-afterglow-1.8:character-profiles-v1';
const ACTIVE_DLC_STORAGE_PREFIX = 'canming-afterglow-1.8:active-scenario-v1:';
...
const WORKSHOP_FENGYUE_STORAGE_KEY = 'canming-afterglow-1.8:workshop-fengyue-items';
...
function ensureGrainLedger(data) {
  const economy = ensureObject(data, '经济');
  const ledger = ensureObject(economy, '粮秣流水');
  if (!ledger.本月入 || typeof ledger.本月入 !== 'object') ledger.本月入 = {};
  if (!ledger.本月出 || typeof ledger.本月出 !== 'object') ledger.本月出 = {};
  return ledger;
}

function sumGrainRecord(record) {
  return roundMarketNumber(entries(record).reduce((sum, [, item]) => sum + Math.max(0, number(item?.数量, 0)), 0));
}

function reconcileGrainLedger(data) {
  if (!data || typeof data !== 'object') return 0;
  const ledger = ensureGrainLedger(data);
  ledger.本月结余 = roundMarketNumber(sumGrainRecord(ledger.本月入) - sumGrainRecord(ledger.本月出));
  return ledger.本月结余;
}
...
function grainLedgerKey(record, label) {
  const base = `${String(label || '粮秣变动').replace(/[./]/g, '·')}·${Date.now().toString(36)}`;
  let key = base;
  let suffix = 2;
  while (Object.hasOwn(record, key)) key = `${base}·${suffix++}`;
  return key;
}

function appendGrainTransaction(data, direction, quantity, options = {}) {
  const amount = roundMarketNumber(Math.max(0, number(quantity, 0)));
  if (!(amount > 0)) return '';
  const ledger = ensureGrainLedger(data);
  const record = direction === 'in' ? ledger.本月入 : ledger.本月出;
  const label = options.label || (direction === 'in' ? '粮秣入库' : '粮秣出库');
  const key = grainLedgerKey(record, label);
  record[key] = {
    数量: amount,
    单位: options.unit || '石',
    日期: options.date || get(data, '世界运转.当前日期', ''),
    类别: options.category || (direction === 'in' ? '入库' : '出库'),
...
function consumeStoredArmyGrain(data, required, options = {}) {
  ...
  appendGrainTransaction(data, 'out', need, options);
  return need;
}
...
function consumeArmyGrain(data, required, options = {}) {
  ...
  if (grainConsumed > 0) {
    appendGrainTransaction(data, 'out', grainConsumed, {
      label: options.label || '各营月度军粮',
      date: options.date,
      category: '养军',
      description: options.description || '跨月结算扣除仓储军粮',
    });
  }
  if (silverCovered > 0) {
    appendGrainTransaction(data, 'in', silverCovered, {
      label: '折银紧急购粮',
      date: options.date,
      category: '采买',
      description: `军粮不足，支出白银 ${silverSpent} 两临时补粮`,
    });
    appendGrainTransaction(data, 'out', silverCovered, {
      label: '折银补粮发放',
      date: options.date,
      category: '养军',
      description: '临时购得军粮当月即行发放',
    });
  }
  return {
    required: need,
...
function doSettlementInPlace(variables, options = {}) {
  ...
  const grainLedger = ensureGrainLedger(data);
  if (!flow.月入 || typeof flow.月入 !== 'object') flow.月入 = {};
  if (!flow.月出 || typeof flow.月出 !== 'object') flow.月出 = {};
  ...
  const result = {
    ...
    grainIncome: applyArmy ? grainIncomeBeforeClear : 0,
    grainOutcome: applyArmy ? grainOutcomeBeforeClear : 0,
  };
...
function writeSettlementRecord(data, result, isAuto) {
  ...
  economy.上次结算 = {
    ...
    粮秣流入: result.grainIncome || 0,
    粮秣流出: result.grainOutcome || 0,
...
function renderSettleSnapshot(s) {
  ...
  if (s.粮秣流入 || s.粮秣流出) lines.push(`粮秣流水 入 ${s.粮秣流入 || 0} 石 · 出 ${s.粮秣流出 || 0} 石`);
  if (s.士气变动 || s.后勤变动) {
    lines.push(
...
function renderGrainLedgerEntries(record, direction) {
  const items = entries(record).slice(-12).reverse();
  if (!items.length) return `<p class="cm-empty">暂无粮秣${direction === 'in' ? '入库' : '出库'}记录。</p>`;
  return `<div class="cm-grain-ledger-list">${items
    .map(
      ([name, item]) => `<article class="cm-grain-entry ${direction}">
        <span class="cm-grain-sign">${direction === 'in' ? '+' : '−'}</span>
        <div><b>${html(String(name).split('·')[0])}</b><small>${html(item.日期 || '未载日期')} · ${html(item.类别 || '其他')}</small>${item.说明 ? `<p>${html(item.说明)}</p>` : ''}</div>
        <strong>${roundMarketNumber(item.数量 || 0)}${html(item.单位 || '石')}</strong>
      </article>`,
    )
    .join('')}</div>`;
}
...
function renderMoney() {
  ...
  const grainLedger = ensureGrainLedger(statData);
  const grainStock = availableArmyGrain(statData);
  const grainIncome = sumGrainRecord(grainLedger.本月入);
  const grainOutcome = sumGrainRecord(grainLedger.本月出);
  ...
  return `${renderMoneyViewSwitch()}
    ${foldGroup(
      ...
      <p><b>粮秣簿</b>——市集购粮、军令预扣和跨月军粮会自动入账；剧情中的缴获、赈济与损耗由 AI 据实记录。</p>
      <p><b>一句话</b>：日常走私库，大事记流水，粮秣看入出，跨月扣军费军粮。</p>`,
    )}
    ...
    ${card(
      '粮秣簿',
      `<div class="cm-grain-ledger-head">
        <div><small>可食军粮</small><strong>${grainStock} 石</strong></div>
        <div><small>本月粮入</small><strong class="in">+${grainIncome}</strong></div>
        <div><small>本月粮出</small><strong class="out">−${grainOutcome}</strong></div>
        <div><small>可支撑</small><strong>${grainRunway == null ? '无养军消耗' : `${roundMarketNumber(grainRunway, 1)} 月`}</strong></div>
      </div>
      <div class="cm-grid two cm-grain-columns">
        <section><h4>粮秣入库</h4>${renderGrainLedgerEntries(grainLedger.本月入, 'in')}</section>
        <section><h4>粮秣出库</h4>${renderGrainLedgerEntries(grainLedger.本月出, 'out')}</section>
      </div>`,
    )}
    <div class="cm-grid two">
      ${card(
...
async function buyMarketItem(itemId, requestedQuantity, currency) {
  ...
  try {
    ...
    if (item.category === '粮食' && item.unit === '石') {
      appendGrainTransaction(data, 'in', quantity, {
        label: `市集购入${item.name}`,
        category: '采买',
        description: `状态栏市集购入，支出 ${quote.text}`,
      });
    }
