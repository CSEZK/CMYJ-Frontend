...
天下档案的主副本仍存放在聊天变量 `cmyj_world_engine_v1`，同时按聊天 ID 双写到天下演化专属脚本变量备份。

1. `_storageRevision` 是独立于世界演化 `revision` 的单调存储修订号，用于识别其他脚本回写的旧聊天变量快照；
2. 主副本缺失或落后时，以独立备份为准并自动恢复，不把短暂缺失初始化为空档案；
3. 后台完整性检查每两秒执行一次，使没有附带酒馆事件的整表覆盖也能被发现；
4. 用户主动清空时写入删除标记；清空前旧副本之后被写回时仍会被拦截；
5. 清空后的下一次合法演化会越过删除标记，建立新的存储世代。

独立备份只承担数据容灾，不参与提示词、演化决策或增量合并。
