...
assert.match(v19StatusbarSource, /async function saveScenarioCharacter\(characterName, character\)/);
assert.match(v19StatusbarSource, /async function readSavedScenarioCharacter\(characterName\)/);
assert.match(v19StatusbarSource, /body\.append\(`alternate_greetings\[\$\{index\}\]`, greeting\)/);
assert.match(v19StatusbarSource, /append\('extensions', JSON\.stringify\(extensions\)\)/);
assert.match(v19StatusbarSource, /application\/x-www-form-urlencoded;charset=UTF-8/);
