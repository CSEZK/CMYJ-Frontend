...
assert.match(releaseWorldEngineSource, /isOfficialDeepSeekApi\(customApi\)/);
assert.match(releaseWorldEngineSource, /shouldFallbackFromJsonSchema\(error\)/);
assert.match(releaseWorldEngineSource, /usePromptJsonSchema \? \{\} : \{ json_schema: schema \}/);
...
assert.match(releaseWorldEngineSource, /VERSION = '1\.7\.3'/);
